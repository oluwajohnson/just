...
!(function (e, t) {
       ...
       for (var u = 0; u < c.dom7LiveListeners.length; u += 1)
        r
         ? c.dom7LiveListeners[u].listener === r &&
           c.removeEventListener(l[d], c.dom7LiveListeners[u].proxyListener, n)
      ...
      for (var h = 0; h < c.dom7Listeners.length; h += 1)
       r
        ? c.dom7Listeners[h].listener === r &&
          c.removeEventListener(l[d], c.dom7Listeners[h].proxyListener, n)
 ...
 var h = {
   ...
   updateActiveIndex: function (e) {
    ...
    if (
     ((t =
      r.indexOf(i) >= 0 ? r.indexOf(i) : Math.floor(c / n.slidesPerGroup)) >=
  ...
  g = {
   ...
   slideToClickedSlide: function () {
    var t,
     ...
     s = a.$wrapperEl,
     r =
      'auto' === i.slidesPerView ? a.slidesPerViewDynamic() : i.slidesPerView,
     n = a.clickedIndex
    if (i.loop) {
   ...
   return (
    ...
    (a.prototype.destroy = function (e, t) {
     ...
     a.emit('beforeDestroy'),
      ...
      !1 !== e &&
       ((a.$el[0].swiper = null), a.$el.data('swiper', null), l.deleteProps(a)),
      (a.destroyed = !0)
    }),
  ...
  N = {
   update: function (e) {
    ...
    for (var C = 0; C < c.length; C += 1)
     C >= w &&
      C <= y &&
      (void 0 === p || e ? E.push(C) : (C > p && E.push(C), C < d && T.push(C)))
    E.forEach(function (e) {
    ...
    if (
     (l || d
      ...
      : i.isHorizontal()
        ...
        : ((d = o), (l = '0')),
     (l =
      l.indexOf('%') >= 0 ? parseInt(l, 10) * a * n + '%' : l * a * n + 'px'),
  ...
  Z = {
   ...
   onGestureChange: function (e) {
    ...
    s.$imageEl &&
     ...
     i.scale < a.minRatio &&
      (i.scale = a.minRatio + 1 - Math.pow(a.minRatio - i.scale + 1, 0.5)),
    ...
    if (
     ...
     if (!(n < i.slideWidth && o < i.slideHeight)) {
      ...
      e.preventDefault(),
       ...
       (r.x =
        (s.touchesCurrent.x - r.prevPositionX) / (Date.now() - r.prevTime) / 2),
       (r.y =
        (s.touchesCurrent.y - r.prevPositionY) / (Date.now() - r.prevTime) / 2),
  ...
  J = {
   ...
   load: function () {
    function t(e) {
     if (l) {
      if (
       s.children('.' + r.slideClass + '[data-swiper-slide-index="' + e + '"]')
        .length
      )
       return !0
  ...
  te = {
    ...
    return (
     (this.interpolate = function (e) {
      return e
       ? ((s = a(this.x, e)),
         (i = s - 1),
         ((e - this.x[i]) * (this.y[s] - this.y[i])) / (this.x[s] - this.x[i]) +
          this.y[i])
       : 0
     }),
     this
   ...
   setTranslate: function (e, t) {
    function a(e) {
     var t =
      e.rtl && 'horizontal' === e.params.direction ? -r.translate : r.translate
    ...
    p.shadow &&
     (c
      ? (0 === (t = s.find('.swiper-cube-shadow')).length &&
         ((t = e('<div class="swiper-cube-shadow"></div>')), s.append(t)),
        t.css({ height: n + 'px' }))
